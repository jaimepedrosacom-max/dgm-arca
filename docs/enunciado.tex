...
\begin{center}
  {\Large\bfseries\color{arcablue} Práctica ARCA}\\[0.6ex]
  {\large Agente con Razonamiento, Conocimiento y Acción}\\[1.2ex]
...
Sabemos que vais a programar con ayuda de agentes de código, y no os lo vamos a prohibir.
...
en la defensa oral preguntamos a cualquiera de los tres sobre cualquier parte del código.
...
\textbf{En esta fase no se usa \code{smolagents}.} Queremos ver vuestro bucle. En la fase 4 sí
lo permitimos, como contraste.
...
\textbf{La corrección se hace llamando a vuestra API.} No leemos vuestros notebooks para saber
si funciona: lanzamos peticiones. Por eso los contratos de entrada y salida están fijados en
...
\textbf{Un ejemplo de lo que esperamos ver.}
...
y nos pasáis la URL. Antes de pasárnosla, probad los cuatro endpoints desde \code{/docs} y
comprobad que \code{/health} los marca como \code{ready}. Rellenad también \code{ARCA\_TEAM}
y \code{ARCA\_DOMAIN} en vuestro \code{.env}: salen en \code{/health} y nos dicen a quién
estamos corrigiendo. Detalles en \file{api/README.md}.
\end{nota}
...
\toprule
\textbf{Criterio} & \textbf{Peso} & \textbf{Qué miramos} \\
\midrule
Implementación correcta & 30\,\% & El endpoint responde y hace lo que dice. Lo comprobamos con
...
La diferencia entre un trabajo correcto y uno excelente está en el tercer criterio. Queremos
...
\begin{enumerate}
  ...
  \item Elegid tema y escribid la propuesta (\file{docs/00\_propuesta.md}), empezando por la
  sección de la tarea verificable: si esa sección se escribe sola, tenéis tema. Si no se os
  ocurre ninguno, en \file{docs/temas\_ejemplo.md} hay trece desarrollados (farmacia, legal,
  finanzas, desarrollo, social, marketing y ciencia) con su tarea verificable, sus
  herramientas y su corpus.
\end{enumerate}
